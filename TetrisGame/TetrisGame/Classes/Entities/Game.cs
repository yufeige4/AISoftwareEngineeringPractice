...
namespace TetrisGame.Entities
{
    class Game
    {
        ...
        public Game(Canvas board, Shapes.Shape shape)
        {
            this.board = board;
            Moved = false;
        ...
        public Brick[,] BlocksInBoard
        {
            get
            {
                Brick[,] shapeOnBoard = new Brick[board.Size.Width, board.Size.Height];
                for (int j = 0; j < 4; j++)
                {
                    for (int i = 0; i < 4; i++)
                    {
                        if (states[stateIndex][i, j])
                            shapeOnBoard[Location.X + i, Location.Y + j] = new Brick(board.BlockSize, Shape.Color, Shape.SideColor, Shape.TopColor, Shape.BottomColor);
                    }
                }
                return shapeOnBoard;
            }
        ...
        public void Render(Graphics g)
        {
            for (int i = 0; i < 4; i++)
            {
                for (int j = 0; j < 4; j++)
                {
                    ...
                    {
                        Brick b = new Brick(blockSize, Color.Purple, Shape.SideColor, Shape.TopColor, Shape.BottomColor, BorderWidth);
                        if (isCurrent)
